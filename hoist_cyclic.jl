...
function main(W,R,N,Capacity,V,L,U)
    ...
    Q = [Qlamda((V,x)) |> x -> tuple.(eachcol(x)...) |> Set  for x in 1:W]  
    # for (r,i) ∈  Q[1]
    #     println(r,i)
    # end
    ...
    #(5.3)
    @constraint(m,
                [r = 1:R, u = 1:R, i = 1:N[r]-1, j = 1:N[u]; (r != u || i != j)], 
...
for (count,(r,i)) in enumerate(keys(st))
    ...
    # 同じパーツが次のところに移動するときは実線
        lines!(ax, [st[r,i], st[r,i] + E[rt[r,i],rt[r,i+1]]+D[r,i]], [rt[r,i], rt[r,i+1]]; color = :black) 
    end
    ...
    text!(ax,st[r,i], rt[r,i], text="r$(r),i$(i)")
    text!(ax,st[r,i], rt[r,i]-0.15, text="$(st[r,i])")
    global rz1 = r
    global iz1 = i
